...
public class FileController {
    ...
    @Transactional
    List<FileMetadataDto> seedDemoData(@RequestBody DemoUserRequestDto request) {
        UUID userId = resolveUserId(request.userId());
        seedSampleFile(userId, "README.md", "text/markdown; charset=UTF-8", "# Welcome\n\nYour demo account is ready.\n");
        seedSampleFile(userId, "audit-log-sample.json", MediaType.APPLICATION_JSON_VALUE, """
                {"event":"LOGIN","status":"SUCCESS","source":"audit-service"}
                """);
        seedSampleFile(userId, "prometheus-targets.csv", "text/csv; charset=UTF-8", """
                service,status
                auth-service,UP
                api-gateway,UP
                """);
        seedSampleFile(userId, "grafana-dashboard-notes.md", "text/markdown; charset=UTF-8", """
                # Grafana
    ...
    private void seedSampleFile(UUID userId, String fileName, String contentType, String content) {
        if (repository.existsByUserIdAndOriginalName(userId, fileName)) {
            return;
        }
        ...
        metadata.setContent(Base64.getEncoder().encodeToString(bytes));
        metadata.setSizeBytes(bytes.length);
        metadata.setPublic(false);
