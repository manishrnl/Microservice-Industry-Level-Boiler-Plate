import {Clock3, Database, LoaderCircle} from "lucide-react";
import {useEffect, useMemo, useState} from "react";
import {useApiActivityStore} from "../../store/apiActivityStore";

const ApiActivityOverlay = () => {
    const {pendingCount, startedAt, message, detail} = useApiActivityStore();
    ...
    return <div className="pointer-events-none fixed bottom-4 right-4 z-[80] max-w-[calc(100vw-2rem)]">
        <div
            className="flex w-[min(30rem,calc(100vw-2rem))] items-start gap-3 rounded-md border border-white/45 bg-white/70 px-3.5 py-3 text-slate-950 shadow-[0_18px_60px_rgba(15,23,42,0.16)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/70 dark:text-white"
        >
