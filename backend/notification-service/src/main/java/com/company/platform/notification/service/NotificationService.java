...
public class NotificationService {
    ...
    public List<NotificationDto> seedDemoData(DemoUserRequestDto request) {
        ...
        createIfMissing(userId,
                ...
                "Your account is ready with sample files, payments, AI chats, audit events, and observability checks.",
                "/app");
        createIfMissing(userId,
                NotificationCategory.ALERT,
                "Prometheus metrics are live",
                "Actuator metrics are available for scraping so Prometheus and Grafana can show service activity.",
                "/app/admin/observability");
        createIfMissing(userId,
                NotificationCategory.ALERT,
                "Grafana dashboard is ready",
                "Open the observability page after the stack is running to see service health and request metrics.",
                "/app/admin/observability");
        createIfMissing(userId,
                NotificationCategory.PAYMENT,
                "Sample payment history created",
                "Five demo payment records were added so the payments screen has data immediately.",
                "/app/payments");
        createIfMissing(userId,
                NotificationCategory.SYSTEM,
                "Audit trail initialized",
                "Five demo audit events were added, including login, role, session, metrics, and dashboard activity.",
                "/app/admin/audit");
        return list(userId);
    }
    ...
    private void createIfMissing(UUID userId, NotificationCategory category, String title, String message, String actionUrl) {
        if (!repository.existsByUserIdAndTitle(userId, title)) {
            create(userId, category, title, message, actionUrl);
        }
    }
